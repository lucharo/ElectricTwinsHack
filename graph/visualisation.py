...
def _node_coordinates(nodes):
    # PathCollection objects already have get_offsets() method
    # No need to set_offset_position
    return nodes.get_offsets()
...
def _convex_hull_vertices(node_coordinates, community):
    ...
    if len(points) == 1:
        # For 1 point, create a small circle
        center = points[0]
        ...
        # For 2 points, create a slim rectangle
        p1, p2 = points
        vector = p2 - p1
        ...
        # Width of the shape
        width = 0.03  # Adjust as needed
        
        # Create four corners of a slim rectangle
        vertices = np.array([
            p1 - width * perp_vector,
            p1 + width * perp_vector,
            p2 + width * perp_vector,
            p2 - width * perp_vector
        ])
        return vertices
    
    # For 3+ points, use ConvexHull as before
    hull = ConvexHull(points)
    x, y = points[hull.vertices, 0], points[hull.vertices, 1]
    vertices = np.column_stack((x, y))
    return vertices
